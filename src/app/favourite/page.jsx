...
import { getListing, makeFavourite } from "@/lib/lisitng";
...
const Page = () => {
  ...
  const fetchFavourites = async () => {
    ...
    if (
      ...
      if (favouriteCodes.length === 0) {
        toast.warning("No favourites found.", { position: "top-right" });
        setDetailedListings([]); // Clear the list
        return;
      }
  ...
  const removeFromFavourite = async (farmhouseCode) => {
    ...
    if (!userId) {
      toast.error("Login to Remove from Favourite", { position: "top-right" });
      return;
    }

    setIsRemoving(farmhouseCode); // Show loading state for this specific item

    try {
      const response = await makeFavourite(farmhouseCode, userId, "DELETE");

      if (
        response?.status === true &&
        response?.returnMessage === "Record Delete Successfully."
      ) {
        toast.success("Removed from favourites!", { position: "top-right" });
      ...
      {detailedListings.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">
            Start adding some farmhouses to your favourites!
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex justify-between items-center px-4 py-2">
                ...
                <button
                  ...
                  onClick={() => removeFromFavourite(data.farmHouseCode)}
